...
export function OverlayViewer({ overlayId }: OverlayViewerProps) {
  ...
  const [spotifyData, setSpotifyData] = useState<{ isPlaying: boolean; title?: string; artist?: string; albumArt?: string }>({ isPlaying: false });
  ...
  useEffect(() => {
    ...
    fetchSpotify();
    const spotifyInterval = setInterval(fetchSpotify, 5000);
            ...
            <div className="absolute top-[8px] left-[8px] right-[8px] h-[42px] bg-gradient-to-r from-slate-900/95 via-slate-800/95 to-slate-900/95 backdrop-blur-md border border-white/10 rounded-3xl flex items-center justify-between px-6 gap-4">
              ...
              <div className="flex items-center gap-2 min-w-[180px] max-w-[220px] overflow-hidden">
                {spotifyData.albumArt ? (
                  <img src={spotifyData.albumArt} alt="" className="w-6 h-6 rounded flex-shrink-0" />
                ) : (
                ...
                <div className="flex flex-col overflow-hidden min-w-0">
                  <span className="text-white text-[11px] font-bold whitespace-nowrap overflow-hidden text-ellipsis" style={{fontFamily: 'Rubik, sans-serif'}}>{spotifyData.isPlaying && spotifyData.title ? spotifyData.title : 'Not Playing'}</span>
                  <span className="text-green-400 text-[10px] font-medium whitespace-nowrap overflow-hidden text-ellipsis" style={{fontFamily: 'Rubik, sans-serif'}}>{spotifyData.isPlaying && spotifyData.artist ? spotifyData.artist : 'Spotify'}</span>
                </div>
              </div>
